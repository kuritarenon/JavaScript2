...
const getNearestCorner = (rect, x, y) => {
  ...
  return corners.reduce((a, b) => {
    ...
    //三項演算子（もしda<dbがtureだったらa,じゃなかったらb）
    return da < db ? a : b;
  });
};
...
function createRipple(parent, x, y, size) {
  ...
  //作った<span class="riple"></span>
  ripple.classList.add("ripple");
  //<span class="riple" style="left:???""></span>
  //postionで動く要素、波紋の出どころを調整しています
  ripple.style.left = `${x - size / 2}px`;
  ripple.style.top = `${y - size / 2}px`;
  //親要素に追加する（append）　<span class="riple" style="left:???""></span>
...
rippleButton.addEventListener("click", (e) => {
  ...
  //入った変数の値を更に関数に入れて、戻ってきた値がrepple
...
// カードを傾ける
//１.function 関数名
//2.const 関数名　＝function(){}*関数式
//3.関数名（）=>｛｝＊アロー関数
//4.const 関数名　＝（）｛｝＊アロー関数を使った関数式
//関数の定義
//関数の定義（関数式）＋引数付き
...
tiltCard.addEventListener("mousemove", (e) => {
  ...
  //この関数は、要素の中でマウスの位置を「-0.5〜0.5」の範囲に変換します
  //戻された値が、分割代入
